...
【交付模式】
  先做 {delivery_mode_label}：搜 {delivery_mode.search_pool} 人，给你 {delivery_mode.output_target} 校准；本轮 {connect_target_text}。
...
- 交付模式必须渲染成"小样本校准"或"直接扩池"；本轮不发消息时写"不发 Connect"，不要暴露内部枚举值。
